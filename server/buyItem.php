...
$st = "fail";
if($itemPrice){
		$myCoins = $odb -> prepare("SELECT coins FROM users WHERE username = :username AND password = :password");
        ...
        if($myCoins != null){
                ...
                if(!$data){
                        if((int)$itemPrice <= (int)$myCoins){
								$UpdateData = $odb -> prepare("UPDATE users SET coins = coins - :item WHERE username = :user");
